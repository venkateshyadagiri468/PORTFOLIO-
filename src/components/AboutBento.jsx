...
import { Code, Globe, Shield, Terminal, Cpu, CheckCircle2, Zap, Layers, Sparkles } from 'lucide-react';
    ...
    <section id="about" className="py-24 px-6 md:px-12 max-w-7xl mx-auto relative z-10">
      ...
      <div className="flex flex-col gap-3 mb-16 text-center max-w-3xl mx-auto">
        <span className="text-xs font-mono uppercase tracking-widest text-indigo-400 font-bold flex items-center justify-center gap-2">
          <Terminal className="w-4 h-4 text-cyan-400" /> About & Process
        </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-8 glass-panel p-8 rounded-3xl border border-indigo-500/20 glass-panel-hover flex flex-col justify-between">
          <div className="flex flex-col gap-4">
            <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-400">
              <Code className="w-6 h-6" />
            </div>
            <h3 className="text-2xl font-bold text-white">Full-Stack Engineering with Precision</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-6 border-t border-slate-800/80 mt-6">
            <div>
              <span className="text-2xl md:text-3xl font-extrabold text-white font-mono">9+</span>
              <p className="text-xs text-slate-400">Production Repositories</p>
            </div>
            <div>
              <span className="text-2xl md:text-3xl font-extrabold text-indigo-400 font-mono">15+</span>
              <p className="text-xs text-slate-400">REST Endpoints Built</p>
            </div>
            <div>
              <span className="text-2xl md:text-3xl font-extrabold text-cyan-400 font-mono">B.Tech</span>
              <p className="text-xs text-slate-400">CSE Graduate 2025</p>
            </div>
          </div>
        </div>

        {/* Card 2: Remote / Global Badge (Col 4) */}
        <div className="md:col-span-4 glass-panel p-8 rounded-3xl border border-indigo-500/20 glass-panel-hover flex flex-col justify-between bg-gradient-to-br from-indigo-950/40 via-slate-900/60 to-slate-950/80">
          <div className="flex flex-col gap-4">
            <div className="w-12 h-12 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center text-cyan-400">
              <Globe className="w-6 h-6" />
            </div>
            <h3 className="text-xl font-bold text-white">Global Remote Ready</h3>
          ...
          <div className="pt-4">
            <div className="flex items-center gap-2 text-xs font-mono text-cyan-400 bg-cyan-500/10 px-3 py-2 rounded-xl border border-cyan-500/20">
              <Sparkles className="w-4 h-4" /> Pitchmatter (Dubai Remote)
            </div>
          </div>
        </div>

        {/* 4 Process Cards (Col 12 Grid) */}
        <div className="md:col-span-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-2">
          {steps.map((s) => (
            <div
              key={s.num}
              className="glass-panel p-6 rounded-2xl border border-slate-800 hover:border-indigo-500/40 transition-all duration-300 flex flex-col gap-3 group"
            >
              <div className="flex justify-between items-center">
                <span className="text-2xl font-mono font-extrabold text-indigo-400 group-hover:text-cyan-400 transition-colors">
                  {s.num}
                </span>
